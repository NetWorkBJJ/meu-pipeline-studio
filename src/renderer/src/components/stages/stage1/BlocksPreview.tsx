    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={onBack}
            className="rounded-md border border-border px-3 py-1.5 text-sm text-text-muted transition-colors hover:bg-surface hover:text-text"
          >
            Voltar
          </button>
          <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-surface text-left text-xs text-text-muted">
              ...
              <th className="px-3 py-2 w-20 text-right">Chars</th>
          ...
          <tbody>
            {storyBlocks.map((block) => (
              <tr key={block.id} className="border-b border-border/50 hover:bg-surface">
                <td className="px-3 py-2 text-text-muted">{block.index}</td>
                <td className="px-3 py-2 text-text">{block.text}</td>
                <td className="px-3 py-2 text-right text-text-muted">{block.characterCount}</td>
